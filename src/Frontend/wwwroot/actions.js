export const SET_CONNECTION_ID = 'SET_CONNECTION_ID';

export const REQUEST_SHIPMENTS = 'REQUEST_SHIPMENTS';
export const RECEIVE_SHIPMENTS = 'RECEIVE_SHIPMENTS';

export const ADD_SHIPMENT_REQUEST = 'ADD_SHIPMENT_REQUEST';
export const ADD_SHIPMENT_SUCCESS = 'ADD_SHIPMENT_SUCCESS';
export const ADD_SHIPMENT_FAILURE = 'ADD_SHIPMENT_FAILURE';

export const ENTER_EDIT_MODE = 'ENTER_EDIT_MODE';
export const EXIT_EDIT_MODE = 'EXIT_EDIT_MODE';

export const UPDATE_SHIPMENT_REQUEST = 'UPDATE_SHIPMENT_REQUEST';
export const UPDATE_SHIPMENT_SUCCESS = 'UPDATE_SHIPMENT_SUCCESS';
export const UPDATE_SHIPMENT_FAILURE = 'UPDATE_SHIPMENT_FAILURE';

export const SIGNALR_CONNECTION_START = 'SIGNALR_CONNECTION_START';
export const SIGNALR_CONNECTION_STOP = 'SIGNALR_CONNECTION_STOP';
export const SIGNALR_CONNECTION_STATE_CHANGED = 'SIGNALR_CONNECTION_STATE_CHANGED';
export const SIGNALR_INVOKE_METHOD = 'SIGNALR_INVOKE_METHOD';